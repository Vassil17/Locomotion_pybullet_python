...
#### Motion planner
class Gait:
    def __init__(self, T = 0.69999, Sx = 0, Sy = 0, Sz = 0, lift = 0.03, T_num = 15, Dsp_ratio=0.1, dt = 0.005, Qudrupedal=True):
        ### go1_parameters
        ...
        self._steplength[2, 0] = 0
        self._steplength[3, 0] = 0
        self._steplength[4,0] = Sx / 2
        ...
        if ((self.bjx1 >= 2) and (j_index <=self._t_end_footstep)):
            ...
            if (self.bjx1 % 2 == 0):  # odd: left support
                ...
                if ((j_index + 1 - round(self.Tx[self.bjx1 - 1, 0] / self.dt)) * self.dt <= self.Td[self.bjx1 - 1, 0]):
                    ...
                    t_plan = np.zeros([3,1])
                    t_plan[0,0] = t_des - self.dt
                    t_plan[1,0] = (self.Td[self.bjx1 - 1, 0] + self.Ts[self.bjx1 - 1, 0]) / 2 + 0.001
                    t_plan[2,0] = self.Ts[self.bjx1 - 1, 0]  + 0.001
